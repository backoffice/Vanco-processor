...
//Modified for Vanco
                if ( $form->_params['payment_method'] ) {
                    $contribParams['payment_instrument_id'] = $form->_params['payment_method'];
                }
                //
      }
    }

  //Modified by BOT for Vanco to display the transaction id with Pending status only for Vanco Processor
	 $paymentprocessorId = CRM_Core_DAO::getFieldValue( 'CRM_Financial_DAO_PaymentProcessor', 'Vanco', 'id', 'name');
        if( $form->_values['payment_processor_id'] == 1 &&  $pending && $result) {
            $contribParams += array(
 ...
 //Modified for Vanco
	if ( $form->_params['payment_method'] ) {
		...
		$paymentProcessorId = CRM_Core_DAO::getFieldValue( 'CRM_Core_DAO_OptionValue', 'ACH', 'value', 'name' );
		if( $paymentProcessorId == $contribParams['payment_instrument_id'] ){
			$pending = true;
		}
	}
	//
